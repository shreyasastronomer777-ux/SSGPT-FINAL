...
const Editor = forwardRef<any, EditorProps>((props, ref) => {
    ...
    const [state, setState] = useState({
        ...
        logo: { src: paperData.schoolLogo, position: paperData.schoolLogo ? 'header-center' as any : 'none' as any, size: 150, opacity: 0.1 },
    ...
    useEffect(() => {
        ...
        setCoEditorMessages([{ id: '1', sender: 'bot', text: "Academic assistant ready. Use professional LaTeX like $x \\times y$. I've been optimized for board-standard generation." }]);
        onReady();
    }, []);
    ...
    useEffect(() => {
        const paginate = () => {
            ...
            const htmlContent = generateHtmlFromPaperData(state.paper, { 
                ...
                isAnswerKey: viewMode === 'answerKey'
            });
            ...
            children.forEach(child => {
                ...
                const elH = el.getBoundingClientRect().height + 15;
            ...
            setPagesHtml(pages.length ? pages : ['<p>Assembling assessment content...</p>']);
            
            setTimeout(() => triggerMathRendering(pagesContainerRef.current), 100);
        };
        paginate();
    }, [state.paper, state.styles.fontFamily, viewMode, showQuestionsInKey, state.logo]);
        ...
        try {
            ...
            for (let i = 0; i < pageElements.length; i++) {
                ...
                const canvas = await html2canvas(el, { 
                    scale: 3.5, // Ultra-high resolution for professional board standards
                    useCORS: true, 
            ...
            const suffix = viewMode === 'answerKey' ? '_Key' : '_Paper';
        ...
        <div className="flex h-full bg-slate-200 dark:bg-gray-900 overflow-hidden relative">
            {isExporting && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-2xl z-[100] flex flex-col items-center justify-center text-white text-center">
                    ...
                    <h2 className="text-2xl font-black tracking-tight">Generating Professional {viewMode === 'answerKey' ? 'Key' : 'Paper'}</h2>
                    <p className="text-slate-400 mt-2 max-w-sm px-4">Processing high-resolution board layout and complex LaTeX assessment expressions...</p>
                </div>
            )}
            <div className="w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col shadow-2xl z-10">
                ...
                <div className="flex-1 overflow-y-auto">
                    {sidebarView === 'toolbar' && <EditorSidebar styles={state.styles} onStyleChange={(k,v) => setState(s=>({...s, styles:{...s.styles, [k]:v}}))} logo={state.logo} watermark={state.watermark} onBrandingUpdate={u=>setState(s=>({...s, ...u}))} onOpenImageModal={() => setImageModalVisible(true)} onUploadImageClick={()=>{}} paperSize="a4" onPaperSizeChange={()=>{}} isAnswerKeyMode={viewMode === 'answerKey'} onToggleShowQuestions={() => setViewMode(v => v === 'answerKey' ? 'questionPaper' : 'answerKey')} />}
            ...
            {isImageModalVisible && <ImageGenerationModal onClose={() => setImageModalVisible(false)} onInsertImage={()=>{}} />}
        </div>
    );
});
