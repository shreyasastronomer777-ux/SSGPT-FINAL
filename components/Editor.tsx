// Fix: Imported missing React hooks and forwardRef to resolve "Cannot find name" errors.
...
const Editor = forwardRef<any, { paperData: QuestionPaperData; onSave: (p: QuestionPaperData) => void; onSaveAndExit: () => void; onReady: () => void; }>((props, ref) => {
    ...
    useEffect(() => {
        ...
        setCoEditorMessages([{ id: '1', sender: 'bot', text: "Ready to refine your assessment for professional export." }]);
        // Delay onReady slightly to ensure ref is established in parent
    ...
    const paginate = useCallback(() => {
        ...
        container.style.width = `${A4_WIDTH_PX - 140}px`; // Accounting for margins
        ...
        let currentPageHtml = ""; 
        let currentHeight = 0;
        const maxPageHeight = A4_HEIGHT_PX - 120; // Internal vertical space
        ...
        setPagesHtml(pages.length ? pages : ['<div style="text-align:center; padding: 100px;">Formatting...</div>']);
        
        // Re-render math after DOM update
        requestAnimationFrame(() => triggerMathRendering(pagesContainerRef.current));
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            for (let i = 0; i < pageElements.length; i++) {
                ...
                triggerMathRendering(el); // Ensure math is fresh
                
                const canvas = await html2canvas(el, { 
                    scale: 2.5, // High quality but mobile-safe
                    useCORS: true, 
        ...
        <div className="flex h-full bg-slate-200 dark:bg-gray-900 overflow-hidden relative">
            {isExporting && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[100] flex flex-col items-center justify-center text-white">
                    <SpinnerIcon className="w-12 h-12 mb-4 text-indigo-400" />
                    <h2 className="text-xl font-bold">Exporting...</h2>
                </div>
            )}
            <div className="hidden lg:flex w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex-col shadow-xl z-10">
            ...
            <main className="flex-1 overflow-auto p-4 sm:p-12 bg-slate-200 dark:bg-slate-950/20" ref={pagesContainerRef}>
                {pagesHtml.map((html, i) => (
                    <div key={i} className="paper-page bg-white shadow-xl mx-auto mb-10 relative overflow-hidden" 
                        style={{ width: A4_WIDTH_PX, height: A4_HEIGHT_PX }}>
                        <div className="paper-page-content prose max-w-none p-[50px] sm:p-[70px] select-text" 
