...
const ChatbotInterface: React.FC<{ onGenerate: (formData: FormData) => void }> = ({ onGenerate }) => {
  ...
  const initChat = useCallback(async () => {
    ...
    try {
      ...
      // Fix: Used 'gemini-3-flash-preview' for basic text tasks as recommended by guidelines.
  ...
  const startLiveSession = async (voice: VoiceOption) => {
    ...
    sessionPromiseRef.current = ai.live.connect({
      // Fix: Followed guidelines for correct Live API model naming and manual PCM decoding.
